...
const Sales: React.FC = () => {
      ...
      if (data.success) {
        if (data.suggestions && data.suggestions.length > 0) {
          ...
          if (data.suggestions.length === 1) {
            // If only one contact found, auto-select it
  ...
  const handleSelectContact = async (contact: Contact) => {
    ...
    // Populate form with selected contact data
    setState(prev => ({
      ...prev,
      customer: {
        ...prev.customer,
  ...
  const handleNewContact = () => {
    ...
    // Reset customer form fields (keep mobile number)
    setState(prev => ({
      ...prev,
      customer: {
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-0 md:p-10 font-sans">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    if (value.length <= 10) {
                      ...
                      if (value.length === 10) {
                        // Don't auto-search, wait for user to trigger
                        setContactSuggestions([]);
                        setShowContactDropdown(false);
                        setSearchMessage('');
                      } else {
            ...
            <div className="md:col-span-2 space-y-6 pt-6 border-t border-gray-100">
              <label className={checkboxLabelClass}>
                <input type="checkbox" className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" checked={state.multiProduct} onChange={(e) => setState(prev => ({ ...prev, multiProduct: e.target.checked }))} />
                ...
                {state.multiProduct && (
                  <>
                    ...
                    <div className="md:col-span-2 animate-in fade-in duration-500">
                      <label className={labelClass}>Salesman Name (3)</label>
